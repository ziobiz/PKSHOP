...
if (!function_exists('pkshop_site_settings_defaults')) {
    function pkshop_site_settings_defaults() {
        return array(
            ...
            /* 회원 약관 페이지(member/agree.php) — 브랜드설정에서 직접 편집 */
            'agree_terms_text' => '',
            'agree_privacy_text' => '',
...
if (!function_exists('pkshop_agree_default_file')) {
    function pkshop_agree_default_file($kind) {
        $map = array(
            'terms' => dirname(__FILE__) . '/agree_defaults/terms.txt',
            'privacy' => dirname(__FILE__) . '/agree_defaults/privacy.txt',
        );
        return isset($map[$kind]) ? $map[$kind] : '';
    }
}

if (!function_exists('pkshop_agree_default_text')) {
    function pkshop_agree_default_text($kind) {
        $path = pkshop_agree_default_file($kind);
...
if (!function_exists('pkshop_agree_terms_text')) {
    /** 이용 약관 / 환불 정책 본문 (브랜드설정 저장값 우선) */
    function pkshop_agree_terms_text($for_admin = false) {
        $saved = trim((string)pkshop_site_setting('agree_terms_text'));
        $text = ($saved !== '') ? (string)pkshop_site_setting('agree_terms_text') : pkshop_agree_default_text('terms');
        if ($for_admin) {
            return $text;
        }
        return pkshop_brand_replace_text($text);
    }
}

if (!function_exists('pkshop_agree_privacy_text')) {
    /** 개인 정보 보호 정책 본문 (브랜드설정 저장값 우선) */
    function pkshop_agree_privacy_text($for_admin = false) {
        $saved = trim((string)pkshop_site_setting('agree_privacy_text'));
        $text = ($saved !== '') ? (string)pkshop_site_setting('agree_privacy_text') : pkshop_agree_default_text('privacy');
        if ($for_admin) {
            return $text;
        }
        return pkshop_brand_replace_text($text);
