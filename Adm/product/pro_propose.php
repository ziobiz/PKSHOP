...
function go_select(i) {
	...
	document.form.action="pro_propose.php";
	document.form.submit();
}

function go_del() {
	document.form.action="pro_theme_del.php?theme=r";
	document.form.submit();
}
...
function go_rank(num) {
	...
	document.form.action="pro_theme_rank.php?theme=r&rank_num="+num;
	document.form.submit();
}
...
function go_add() {
	...
	document.form.action="pro_theme_add.php?theme=r";
	document.form.submit();
}
function all_chk() {
...
<?
$query = "SELECT code,title,theme_r FROM $shop_goods WHERE (code1='$sel_code1' or code2='$sel_code2' or code3='$sel_code3' or code4='$sel_code4') and soldout<>'Y' ORDER BY signdate DESC";
$DB->get($query,$rs,$rn);
...
?>
										<option value="<?=$theme_r.$g_code?>" <?=$oselect?>><?=$title?></option>
...
$query = "SELECT code,code1,code2,code3,title,currnum,pricec,rank_r,code4,order4,No FROM $shop_goods WHERE theme_r='r' ORDER BY rank_r asc,signdate DESC";
...
?>
								<tr align="center"> 
									...
									<td width="332" height="26">
										<a href="pro_info.php?sel_theme=r&code=<?=$code?>&No=<?=$No?>"><?=$title?></a>
									</td>
