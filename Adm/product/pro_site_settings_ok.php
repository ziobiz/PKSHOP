...
$no_trim_keys = array('agree_terms_text', 'agree_privacy_text', 'login_notice_body', 'login_auth_main_text');
foreach ($defaults as $k => $v) {
	if (isset($_POST[$k])) {
		if (is_string($_POST[$k])) {
			$data[$k] = in_array($k, $no_trim_keys, true) ? (string)$_POST[$k] : trim($_POST[$k]);
		} else {
			$data[$k] = $_POST[$k];
		}
	}
