...
for($i=1;$i<2;$i++) {
	...
	$cond = "ss.ordernum=so.ordernum and ss.signdate>".$mkt." and ss.signdate <".$nmkt." and so.status like '배송완료'";	
?>
							<tr bgcolor='#EBF0F4'>
									<td align=center height=30><?=$ii?></td>
<?
	### 카드 매출 ##########################################################
	$query1 = "SELECT ss.ordernum,ss.code,ss.title,ss.money,ss.count,so.kind FROM $shop_sell as ss,$shop_order as so where kind='1' and ss.signdate >1325257200 and $cond order by ss.ordernum desc";
...
<?
	### 온라인 매출 ##########################################################
	$query2 = "SELECT ss.ordernum,ss.code,ss.title,ss.money,ss.count,so.kind FROM $shop_sell as ss,$shop_order as so where kind='2' and $cond order by ss.ordernum desc";
...
<?
	### 전체 판매가격 ##########################################################
	$cond = "ss.ordernum=so.ordernum and ss.signdate>".$mkt." and ss.signdate <".$nmkt." and so.status like '배송%'";	
