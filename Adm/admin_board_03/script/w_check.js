...
function check(fm)
	{
		...
		if(check_select(fm.Title,'등록될 제품코드를')=='wrong'){return false}
...
function check1(fm)
	{
		if(check_blank(fm.Name,'작성자를',1)=='wrong'){return false}
		if(check_blank(fm.Cont,'내용을',5)=='wrong'){return false}
...
function check2(fm)
	{
		if(check_blank(fm.Name,'작성자를',1)=='wrong'){return false}
		if(check_blank(fm.Pass,'비밀번호',4)=='wrong'){return false}
		if(check_blank(fm.Cont,'내용을',5)=='wrong'){return false}
		document.form3.submit();
	}

function check2_h(fm)
	{
		if(check_blank(fm.Name,'작성자를',1)=='wrong'){return false}
		if(check_blank(fm.Pass,'비밀번호',4)=='wrong'){return false}
		if(check_blank(fm.Cont,'내용을',5)=='wrong'){return false}
		document.form2.keynum.value = Cal_Key_num;
		document.form2.submit();
	}