...
function inputkindmock()
 {
        $useragnet = $_SERVER['HTTP_USER_AGENT'];
        $mobileBrower = '/(ipod|ipad|iphone|android|blackberry|symbianos|sch-m\d+|opera mini|windows ce|nokia|sonyericsson|webos|palmos|mobile|up.Browser|opera mobi|windows phone|iemobile|polaris)/';
        $reval = "P";  //기본값 PC
  if(preg_match($mobileBrower, strtolower("$useragnet"))) $reval = "M";  //모바일
  return $reval;
 }
$m_check =inputkindmock();
...
function pay_result_close(){
    ...
    <script language="JavaScript">
        ...
        function paygo() {
            ...
            // }
            if (!document.join.buyername.value) {
                alert('Order please enter firstname. 첫번째 칸에 "본인의 이름"을 기입하여 주십시오.');
                document.join.buyername.focus();
                return;
            }
            if (!document.join.buyername_l.value) {
                alert('Order please enter lastname. 두번째 칸에 "본인의 성"을 기입하여 주십시오.');
                document.join.buyername.focus();
                return;
            }
            if (!document.join.post.value) {
                alert('Please enter the zip code of the orderer. 배송지의 "우편번호"을 작성하여 주십시오');
                document.join.post1.focus();
                return;
            }
            if (!document.join.addr1.value) {
                alert('Enter the address of the orderer. 회원정보의 주소가 없다면 직접 입력하여 주십시오');
                document.join.addr1.focus();
                return;
            }
            // if (!document.join.city.value) {
            ...
            if (!document.join.email.value) {
                alert('Enter the orderer\'\s email. 주문자의 "이메일"을 작성하여 주십시오.');
                document.join.email.focus();
                return;
            }

            if (!document.join.recvname.value) {
                alert('Enter the recipient\'\s name. 수령인의 "성명"을 작성하여 주십시오');
                document.join.recvname.focus();
                return;
            }
            if (!document.join.recvname_l.value) {
                alert('Enter the recipient\'\s lastname. 수령인의 "성"을 작성하여 주십시오');
                document.join.recvname.focus();
                return;
            }
            ...
            if (document.join.paymentkind.value == 5) {
                ...
                if (document.join.paymentkind.checked == true) {
                    if (document.join.in_name.value == "") {
                        alert('Please enter the name of the depositor. 송금자의 이름을 작성하여 주십시오');
...
<body>
    <div class="wrap">
		...
		//-->
                            ...
                            if ($_SESSION["member_id"] =="test1234") {
                                $total_settle_num = 100;
                            }else{
				...
				<td><input type="text" value="<?= number_format($json_balance['total_SP']) ?>" readonly class="input_name"></td>
                            ...
                            <tr>
                                <th>Name<div class="sp3"></div>(이름)</th>
                                <td><input type="text" name="buyername" value="<?= $name ?>" class="input_name"></td>
                            </tr>
                            <tr>
                                <th>Last Name<div class="sp3"></div>(성)</th>
                            ...
                            <tr>
                                <th>Address<div class="sp3"></div>(주소)</th>
                                <td>
                                    Zip Code (우편번호)&nbsp;
                                    <input type="text" name="post" value="<?=$json_balance['C_ZIP']?>" id="post" class="input_name">&nbsp;
                                    <!-- <input type="button" value="Find Address" class="find_address" > -->
                                    <div class="sp5"></div>
                                    Delivery Address (주소) &nbsp;
                                    <input type="text" style="width:50%" name="addr1" value="<?=$json_balance['C_ADDR']?> <?=$json_balance['C_ADDR2']?>" id="addr1" class="input_addr">
                                </td>
                            </tr>
                            <!-- <tr>
                                <th>City</th>
                            ...
                            <tr>
                                <th>E-mail<div class="sp3"></div>(이메일)</th>
				...
				<td><input type="text" class="input_tel">&nbsp;-&nbsp;<input type="text" class="input_tel">&nbsp;-&nbsp;<input type="text" class="input_tel"></td>
                            ...
                            <tr>
                                <th>Cell phone<div class="sp3"></div>(휴대전화)</th>
                        ...
                        <div class="order_title">
                               Shipping Information (배송정보)&nbsp;&nbsp; <span class="font_12 font_thin c_gary">
                        <div class="sp10"></div>
                         <span style="color:#c3070b"><B>Is the delivery address of the orderer the same?</B></span>
                         <div class="sp10"></div>
                         <span style="color:#c3070b"><B>(주문자의 배송지가 동일 합니까 ?)</B></span>&nbsp;&nbsp;&nbsp;&nbsp;<input name="buytype" type="radio" value="radiobutton" onClick="sync_data(1);">Yes.
                         <input type="radio" name="buytype" value="radiobutton" onClick="sync_data(2);">No, I don't. 
                         <div class="sp15"></div>
                          <b>Recipient Information (수취인 정보)</b>
                        </div>
                        ...
                        <table class="order_table">
                            <tr>
                                <th>Name<div class="sp3"></div>(성명)</th>
                            ...
                            <tr>
                                <th>Address<div class="sp3"></div>(주소)</th>
                                <td>
                                    PZip Code (우편번호)&nbsp;<input type="text" name="rpost" id="rpost" class="input_name">&nbsp;
                                    <!-- <input type="button" value="Find Address" class="find_address" onClick="openDaumPostcode1('rpost','raddr1');"> -->
                                    <div class="sp5"></div>
                                    Delivery Address (주소) &nbsp;<input type="text" style="width:50%" name="raddr1" id="raddr1" class="input_addr">
                                </td>
                            </tr>
                            <!-- <tr>
				...
				<td><input type="text" class="input_tel">&nbsp;-&nbsp;<input type="text" class="input_tel">&nbsp;-&nbsp;<input type="text" class="input_tel"></td>
                            ...
                            <tr>
                                <th>Requirements<div class="sp3"></div>(요구사항)</th>
				...
				<td>
					<input name="usepoint" type="number" class="userpoint" id="userpoint" class="input_name"  placehoder="Please enter the shopping point you want to use." id="textfield7" size="15">
                            ...
                            <tr>
                                <th>Amount<div class="sp3"></div>(결제금액)</th>
                                <td>
				...
				<td>
					<input name="bank" type="text"  id="bank" class="input_name"  placehoder="입금할 은행 계좌번호를 입력해주세요" id="textfield7" size="30">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <? if ($onlyP > 0) { ?>
                                        ...
                                        InT (InT 결제)
                                    <? } else { ?>
                                        <input name="paymentkind" class="paymentkind" type="radio" value="2" checked>
                                        BANK (계좌이체)
                                        <input name="paymentkind" class="paymentkind" type="radio" value="1">
                                        CARD (카드결제)
                                    <? } ?>
 					...
 					무통장결제   -->
                        ...
                        <div class="view_btn">
                            <input type="button" value="Cancellation 취소" class="cart_btn01" onclick="location.href='./cart.php'">

                            <input type="button" value="Order 결제" class="cart_btn03 bank" <? if ($out111 == "Y") { ?>onclick="javascript:sold_out()" <? } else { ?>onclick="javascript:paygo();" <? } ?>>&nbsp;
                            <input type="button" value="Order 결제" class="cart_btn03 card" id="cardBtn" style="display:none">&nbsp;
                        </div>
                        	<!-- 옵션 --> 
                            <?
                            
                                $merchantKey	= "L5imiEYu5wHx1I0dII+ySDqbb0CyWZsrAgwHKUMTPoWgOVkvZQe6vV2MO4bMxTULq5+CidDVJcuWmdzwrpF0wA=="; 		
                                $merchantID		= "afgc00001m";					// 상점아이디
                                ...
                                $returnUrl		= "//pentakleva.shop/cart/finish.php"; 			// 결과페이지(절대경로)
	...
	<input type="hidden" name="mbsReserved" value="<?=$_SESSION['member_id']?>&&<?=$_SESSION['member_code']?>">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#cardBtn").click(function(event) { //팝업 Open 버튼 클릭 시 
            // if("<?=$m_check?>" == "M"){
            //     alert("Card payment is not available for mobile.Please use the PC\r\n(모바일은 카드 결제를 사용할수없습니다.PC로 이용해주세요)");
            //     return false;
            // }
            doPaySubmit();
            // pay();
        ...
        var pay = function() {
            ...
            if (!document.join.buyername_l.value) {
                alert('Order please enter lastname (첫째칸에 성명을 둘째칸에 성을 기입하여 주십시오');
                document.join.buyername.focus();
                return;
            }
            if (!document.join.post.value) {
			...
			*/
            ...
            if (!document.join.recvname.value) {
                alert('Enter the recipient\'\s firstname. 수취인의 "성명"을 작성하여 주십시오');
                document.join.recvname.focus();
                return;
            }

            if (!document.join.recvname_l.value) {
                alert('Enter the recipient\'\s lastname. 수취인의 "성"을 작성하여 주십시오');
                document.join.recvname.focus();
                return;
            }
