...
$StoreNm = iconv("utf-8","euc-kr",$StoreNm);
$ProdNm = iconv("utf-8","euc-kr",$ProdNm);
$OrdAddr = iconv("utf-8","euc-kr",$OrdAddr);
$DlvAddr = iconv("utf-8","euc-kr",$DlvAddr);
$RcpNm = iconv("utf-8","euc-kr",$RcpNm);
$Remark = iconv("utf-8","euc-kr",$Remark);
$OrdNm = iconv("utf-8","euc-kr",$OrdNm);
...
********************************************************************************/
	...
	****************************************************************************/
	...
	$agspay->SetValue("AgsPayHome",$_SERVER[DOCUMENT_ROOT]."/pc/cart/php");			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	$agspay->SetValue("ProdNm",trim($ProdNm));			//상품명
	...
	/*신용카드사용*/
	$agspay->SetValue("OrdNm",trim($OrdNm));			//주문자명
	$agspay->SetValue("OrdPhone",trim($_POST["OrdPhone"]));		//주문자연락처
	$agspay->SetValue("OrdAddr",trim($OrdAddr));		//주문자주소 가상계좌추가
	$agspay->SetValue("RcpNm",trim($RcpNm));			//수신자명
	$agspay->SetValue("RcpPhone",trim($_POST["RcpPhone"]));		//수신자연락처
	$agspay->SetValue("DlvAddr",trim($DlvAddr));		//배송지주소
	$agspay->SetValue("Remark",trim($Remark));			//비고
	...
	****************************************************************************/
	...
	$rOrdNm =  iconv("utf-8","euc-kr",$rOrdNm);
	$rProdNm = iconv("utf-8","euc-kr",$rProdNm);
...
<form name=frmAGS_pay_ing method=post action=finish.html>
<!-- 포인트 사용  -->
<?$Signdate_kk2 = date("Y-m-d H:i:s");?>
<input type=hidden name=usepoint value=<?=$usepoint?>>
<input type=hidden name=Signdate_kk2 value=<?=$Signdate_kk2?>>
...
<input type=hidden name=rStoreId value="<?=$agspay->GetResult("StoreId")?>">	<!-- 상점아이디 -->
<input type=hidden name=rOrdNo value="<?=$agspay->GetResult("OrdNo")?>">		<!-- 주문번호 -->
<input type=hidden name=rProdNm value="<?=$agspay->GetResult("ProdNm")?>">		<!-- 상품명 -->
<input type=hidden name=rAmt value="<?=$agspay->GetResult("Amt")?>">			<!-- 결제금액 -->
